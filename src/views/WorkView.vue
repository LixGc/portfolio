<template>
    <section class="min-h-screen w-full" ref="workSection" id="work-section">
        <div class="relative flex py-5 items-center"
            ...
            <h1 class="text-3xl font-bold pr-2 sm:pr-5">💻 My Projects</h1>
            <div class="flex-grow border-t border-black dark:border-white border-1"></div>
            <a :href="portfolio.archiveLink" class="pl-2 sm:pl-5 hover:text-link-color text-right text-sm">Visit Github</a>
        </div>
        <div
            :class="['flex flex-col space-y-12 mb-36 transition-all motion-reduce:transition-none duration-500 delay-300', visible ? 'translate-y-0 opacity-1 blur-0' : 'translate-y-4 opacity-0 blur-sm']">
            <WorkUnit v-for="(project, index) in projInitial" :key="index" :project="project" />
            <WorkUnit v-if="showMore" v-for="(project, index) in content" :key="index" :project="project" />

            <button v-show="showPreview" @click="toggleShowMore"
